...
LOAD CSV WITH HEADERS FROM "file:///C:/Users/ahamadam/dev/workspace/neo4j-neo4j_ogm/src/main/resources/cypher/csv/lineups.csv" AS csvLine
...
FOREACH(n IN (CASE csvLine.team WHEN "home" THEN [1] else [] END) |
	FOREACH(o IN (CASE csvLine.type WHEN "starting" THEN [1] else [] END) |
		...
		MERGE (player)-[:STARTED]->(stats:STATS)-[:IN_MATCH]->(match)
	)

	FOREACH(o IN (CASE csvLine.type WHEN "sub" THEN [1] else [] END) |
		MERGE (match)-[:HOME_TEAM]->(home)
		MERGE (player)-[:SUBSTITUTE]->(stats:STATS)-[:IN_MATCH]->(match)
	)	
)
